...
// ─────────────────────────────────────────────────
const OUTLET_D_ID = "a1b2c3d4-e5f6-4a7b-8c9d-0e1f2a3b4c5d";

const ADMIN_D_ID = "b1c2d3e4-f5a6-4b7c-8d9e-0f1a2b3c4d5e";
const WASH_D_ID = "c1d2e3f4-a5b6-4c7d-8e9f-0a1b2c3d4e5f";
const IRON_D_ID = "d1e2f3a4-b5c6-4d7e-8f9a-0b1c2d3e4f5a";
const PACK_D_ID = "e1f2a3b4-c5d6-4e7f-8a9b-0c1d2e3f4a5b";
const CUST_D_ID = "f1a2b3c4-d5e6-4f7a-8b9c-0d1e2f3a4b5c";
const WALKIN_D_ID = "a2b3c4d5-e6f7-4a8b-9c0d-1e2f3a4b5c6d";
const WALKIN_ADDR_ID = "b3c4d5e6-f7a8-4b9c-8d1e-2f3a4b5c6d7e";

// Items
const ITEM_KAOS_ID = "1a2b3c4d-5e6f-4a7b-8c9d-1e2f3a4b5c6d";
const ITEM_CELANA_ID = "2b3c4d5e-6f7a-4b8c-9d0e-2f3a4b5c6d7e";
const ITEM_HANDUK_ID = "3c4d5e6f-7a8b-4c9d-8e1f-3a4b5c6d7e8f";
const ITEM_SELIMUT_ID = "4d5e6f7a-8b9c-4d0e-9f2a-4b5c6d7e8f9a";
const ITEM_KEMEJA_ID = "5e6f7a8b-9c0d-4e1f-8a3b-5c6d7e8f9a0b";

// Orders
const ORDER_D1_ID = "6f7a8b9c-0d1e-4f2a-8b4c-6d7e8f9a0b1c";
const ORDER_D2_ID = "7a8b9c0d-1e2f-4a3b-8c5d-7e8f9a0b1c2d";
const ORDER_D3_ID = "8b9c0d1e-2f3a-4b4c-9d6e-8f9a0b1c2d3e";
const ORDER_D4_ID = "9c0d1e2f-3a4b-4c5d-ae7f-9a0b1c2d3e4f";
const ORDER_D5_ID = "0d1e2f3a-4b5c-4d6e-bf8a-0b1c2d3e4f5a";
const ORDER_D6_ID = "1e2f3a4b-5c6d-4e7f-8a9b-1c2d3e4f5a6b";
const ORDER_D7_ID = "2f3a4b5c-6d7e-4f8a-9b0c-2d3e4f5a6b7c";
const ORDER_D8_ID = "3a4b5c6d-7e8f-4a9b-8c1d-3e4f5a6b7c8d";
...
async function main() {
  ...
  await ensureOrderItems(ORDER_D2_ID, [
  ...
  const orderD3Items = await ensureOrderItems(ORDER_D3_ID, [
  ...
  const orderD4Items = await ensureOrderItems(ORDER_D4_ID, [
    { item_id: kaos.id, quantity_initial: 4 },
    { item_id: celana.id, quantity_initial: 3 },
  ]);
  ...
  await ensureOrderItems(ORDER_D5_ID, [
    { item_id: kaos.id, quantity_initial: 3 },
  ...
  const orderD6Items = await ensureOrderItems(ORDER_D6_ID, [
    { item_id: celana.id, quantity_initial: 2 },
  ...
  const existingD6WashSummary = await prisma.stationSummary.findMany({
    where: { order_id: ORDER_D6_ID, station: "washing" },
  });
  if (existingD6WashSummary.length === 0) {
    for (const oi of orderD6Items) {
      await prisma.stationSummary.create({
        data: {
          order_id: ORDER_D6_ID,
          item_id: oi.item_id,
          station: "washing",
          latest_quantity: oi.quantity_initial,
        },
  ...
  const existingD6IronLogs = await prisma.orderStationLog.findMany({
    where: { order_id: ORDER_D6_ID, station: "ironing" },
  });
  ...
  const orderD7Items = await ensureOrderItems(ORDER_D7_ID, [
  ...
  const existingD7IronSummary = await prisma.stationSummary.findMany({
    where: { order_id: ORDER_D7_ID, station: "ironing" },
  });
  if (existingD7IronSummary.length === 0) {
    for (const oi of orderD7Items) {
      await prisma.stationSummary.create({
        data: {
          order_id: ORDER_D7_ID,
          item_id: oi.item_id,
          station: "ironing",
          latest_quantity: oi.quantity_initial,
        },
  ...
  const existingD7PackLogs = await prisma.orderStationLog.findMany({
    where: { order_id: ORDER_D7_ID, station: "packing" },
  });
  ...
  const orderD8Items = await ensureOrderItems(ORDER_D8_ID, [
    { item_id: kaos.id, quantity_initial: 4 },
    { item_id: celana.id, quantity_initial: 3 },
  ...
  console.log(`   │  Outlet D ID: ${OUTLET_D_ID}  │`);
  console.log("   └─────────────────────────────────────────────────────┘");
  ...
  console.log(`     GET  /api/admin/mismatch?outletId=${OUTLET_D_ID}`);
  console.log(
    `     GET  /api/admin/mismatch/${ORDER_D4_ID}/washing?outletId=${OUTLET_D_ID}`,
  );
  console.log(
    `     PUT  /api/admin/mismatch/${ORDER_D4_ID}/washing?outletId=${OUTLET_D_ID}`,
  );
  console.log("   workerStation (login as worker first):");
  console.log("     GET  /api/workers/available");
  console.log("     GET  /api/workers/active");
  console.log(`     POST /api/workers/accept/${ORDER_D1_ID}`);
  console.log(`     POST /api/workers/reinput/${ORDER_D2_ID}`);
  console.log(`     POST /api/workers/complete/${ORDER_D3_ID}`);
}
